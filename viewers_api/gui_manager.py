...
class ViewerGUI(tk.Tk):
    def __init__(self):
        ...
        # Отдельный валидатор списков (если нужен)
    ...
    def control_task(self, action):
        ...
        try:
            if action == "stop":
                r = requests.delete(f"{API_URL}/{task_id}", timeout=10)
            elif action == "pause":
                r = requests.post(f"{API_URL}/{task_id}/pause", timeout=10)
            elif action == "resume":
                r = requests.post(f"{API_URL}/{task_id}/resume", timeout=10)
            else:
    ...
    def raid_task(self):
        ...
        def send_raid():
            try:
                ...
                r = requests.post(f"{API_URL}/{task_id}/raid", json=data, timeout=10)
                if r.status_code >= 400:
                    messagebox.showerror("Ошибка", r.text)
                else:
