...
class ViewerBot:
    ...
    async def _get_playback_token(self):
        ...
        async def _query(is_live: bool, *, use_persisted: bool):
            ...
            if use_persisted:
                payload = [{
                    ...
                    "extensions": {
                        "persistedQuery": {
                            "version": 1,
                            # твой старый хэш; если на его стороне кеш пуст — получим PersistedQueryNotFound
            ...
            async with self._session.post(TWITCH_GQL_URL, headers=headers, json=payload, proxy=self.proxy) as resp:
                ...
                # если есть явные ошибки — пробрасываем
                if item0.get("errors"):
                    # вернём специальную метку, чтобы снаружи понять 'PersistedQueryNotFound'
                ...
                if not token_node or "signature" not in token_node or "value" not in token_node:
                    raise RuntimeError(f"GQL missing token node (isLive={is_live}, persisted={use_persisted}): {str(item0)[:300]}")
                return token_node["signature"], token_node["value"]

        async def _query_with_fallback(is_live: bool):
            # 1) пытаемся persisted
            try:
                return await _query(is_live, use_persisted=True)
            except Exception as e1:
                msg = str(e1)
                # Если это PersistedQueryNotFound — пробуем текстовый запрос
                if "PersistedQueryNotFound" in msg:
                    try:
                        return await _query(is_live, use_persisted=False)
                    except Exception as e2:
                        raise RuntimeError(f"Persisted->Text fallback failed (isLive={is_live}). p_err={e1}; t_err={e2}")
                # иначе пробрасываем как есть
                raise

        # Сначала isLive=True, если не взлетело — isLive=False
        try:
    ...
    async def _chat_idle_loop(self, nick: str):
        try:
            while self._running and self._ws:
                await asyncio.sleep(random.randint(120, 260))
                if random.random() < 0.15:
                    with contextlib.suppress(Exception):
                        await self._ws.send(f"PRIVMSG #{self.channel} :Kappa")
        except Exception:
            pass
    ...
    async def _refresh_tokens(self):
        ...
        self._session._default_headers = aiohttp.helpers.CIMultiDict(h)  # да, private, но рабочий лайфхак
        await self._get_playback_token()
        await self._form_playlist_url()
