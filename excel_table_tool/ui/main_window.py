...
from core.ai_router import route_command
...
def launch_gui():
    ...
    def ai_assistant_callback():
        global df_global
        new_df = ai_assistant.run_ai_assistant(df_global, update_status)
        if new_df is not None and not new_df.equals(df_global):
            save_state()
            df_global = new_df
            show_table_preview(df_global, table_frame)
            update_status("AI действие выполнено")
            log("🤖 AI выполнил команду")
            save_history()
    ...
    def on_text_command():
        user_cmd = dialogs.ask_string("AI Команда", "Введите команду:")
        if user_cmd:
            result = route_command(user_cmd, df_global, ui_actions)
            if isinstance(result, str):
                log(result)
            update_status("Команда выполнена")

    ui_actions = {
        "load_files": load_files,
        "load_and_merge_smart": load_and_merge_smart,
        "export_excel": export_to_excel,
        "export_pdf": export_to_pdf,
        "add_column": add_column,
        "clean_data": clean_data,
        "apply_template": apply_template_action,
        "save_template": save_current_as_template,
        "merge_by_key": merge_by_key,
        "show_summary": show_summary,
        "show_histogram": show_histogram,
        "show_pie": show_pie_chart,
        "show_line": show_line_chart,
        "simple_filter": filter_data,
        "advanced_filter": apply_advanced_filter,
        "check_errors": check_errors_and_edit,
        "undo": undo,
        "redo": redo,
        "switch_theme": switch_theme,
        "toggle_autowatch": toggle_autowatch,
        "smart_merge": smart_merge_gui,
        "ai_assistant": ai_assistant_callback
    }
    ...
    tools_menu.add_command(label="Ввести AI-команду", command=on_text_command)
